    ...
    */
    ...
    const svgHelper = async(viewSelector) => {
        ...
        if(historicalPrice.length > 0){
            let temp = await chartObjectHelper(historicalPrice)
    ...
    const taHelperCallback1 = useCallback((index, type) =>  {
        ...
        const previous_dates = async(date, range) => {
            console.log(date)
            return await helpers.prevMarketData(historicalPrice[0].symbol, range, date)
        }
        ...
        const addToTARefs = (e) => {
            ...
            if(e && !activeTACharts.includes(e)) {
                ...
                if(e === 'RSI'){period = 14}
                if(e === 'MACD'){period = 26};
                previous_dates(markedDate, period).then(res => {
                    ...
                    tickerHistoricalDataRange(list[0], res[0], res[res.length - 1], 0)
                }).then(setIsChartLoading(true))
    ...
    }, [list, svgTARef1, activeTARefs, activeTARefs.current, activeTACharts, activeTARefs,svgTaRef, svgTARef1, svgTA1, taRef1, historicalPrice])
    ...
    const rsiHelper1 = useCallback((chartObject, index) => {
        console.log(stateHistoricalPrice)
        console.log(chartObject)
        console.log(index)
        console.log(activeTARefs.current)
        ...
        return rsi_values
    }, [historicalPrice, activeTARefs, activeTACharts ,svgTaRef, svgTARef1, svgTA1, taRef1])
        // let svgTaRef = useRef(null)
    // // const activeTACharts
    // let svgTA1;
    // let svgTARef1 = useRef(null)
    ...
    const rsiChartHelper = async(rsi_values) => {
        ...
        // let svg = d3.select(svgTA1)
            ...
            // .attr("fill", "cream")
            // .attr("fill_opacity", "1")
        // let svg = d3.select(activeTARefs.current)
        let svgTA = d3.create("svg")
            .attr("class", "svgTAEle")
            // .append("TAsvgTA")
            .attr("width", width)
            .attr("height", height)
        ...
        console.log(svgTA.node())
        // console.log(svg.node())
        activeTARefs.current.push(svgTA.node())
        // activeTARefs.current.push(svgTA.node())
        // activeTARefs.current.push(svgTARef1)
        // activeTARefs.current.push(svgTA)
    ...
    const macdHelper1 = async(chartObject, index) => {
        ...
        macd(chartObject).then(res => {
            ...
            const xAxis = d3.axisBottom(xScale)//.tickFormat(xFormat);//.tickValues(xTicks)
            const yAxis = d3.axisLeft(yScale).ticks(height / 100, yFormat);
            let svg = d3.select(svgTA1.current)
                // .append("svg")
                .attr("width", width)
                .attr("height", height)
                .attr("viewBox", [0, 0, width, height])
    ...
    }, [stateHistoricalPrice, historicalPrice, isChartLoading])
    ...
    //is this part actually neccessary? lets try it out
    useEffect(() => {
        console.log(markedDate)
            ...
            <Home.Wrapper>
                ...
                {/* <Home.Wrapper1></Home.Wrapper1> */}
                    ...
                    > */}
                        ...
                        />
                        <svg className = "test"
                            ref = {activeTARefs.current[0]}
                        />


                    {activeTACharts.length > 0 && activeTACharts.map((data, key) => {
                        console.log(key)
                        console.log(data)
                        let className = `${data} taChart`
                        console.log(activeTARefs.current[0])
                        return(
                            <svg
                                className = {className}
                                ref = {activeTARefs.current[key]}
                                key = {key}
                            ></svg>
                        )
